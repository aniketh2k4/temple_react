            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {gurus.map((guru, index) => (
                        <Link to={`/guru/${guru.id}`} key={guru.id} className="group block bg-white rounded-xl shadow-[0_10px_30px_-10px_rgba(0,0,0,0.1)] hover:shadow-2xl hover:shadow-gold-500/10 transition-all duration-700 hover:-translate-y-2 border border-gold-100/50 hover:border-gold-400/50 reveal overflow-hidden relative" style={{ animationDelay: `${(index + 1) * 100}ms` }}>
                            {/* Halo Background */}
                            ...
                            <div className="p-8 text-center relative z-10">
                                ...
                                <div className="relative">
                                    <h3 className="font-display font-bold text-maroon-900 text-xl mb-3 group-hover:text-maroon-700 transition-colors">{guru.name}</h3>
                                    <div className="w-12 h-px bg-gold-300 mx-auto mb-3 group-hover:w-24 transition-all duration-500"></div>
                                    <p className="text-maroon-800/80 font-serif italic text-sm">"{guru.quote}"</p>
                                </div>
                            </div>
                        </Link>
                    ))}
